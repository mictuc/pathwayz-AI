...
def opponentMove(game, state, weightFile):
...
def playGame(game, PAI1, PAI1_starts, opponentWeights):    
    if PAI1_starts:
...
for i in range(numGames):
    ...
    if i % 2 == 0:
        winner = playGame(game, PAI, True, opponentWeights)
        if winner == 'w':
            wins['PAI'] += 1
        elif winner == 'b':
            wins['Opponent'] += 1
        ...
        winner = playGame(game, PAI, False, opponentWeights)
        if winner == 'w':
            wins['Opponent'] += 1
        elif winner == 'b':
            wins['PAI'] += 1
