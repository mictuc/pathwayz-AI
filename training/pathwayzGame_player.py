...
def TDLevaluationFunction(game, board, player, weights):
    features = game.TDLfeatures(board, player)
    #weights = initOpponentWeights('smartPAI_v_smartPAI3100_opt.txt')
    value = sum([features[k] * weights[k] for k in features.keys()])
    if game.isEnd((board, player)):
        return game.utility((board, player)) + value
    return value

def TDLfeaturesMove(game, state, weights):
    _, player = state
    bestScore = -float("inf")
    options = []
    actions = game.actions(state)
    for action in actions:
        newState = game.simulatedMove(state, action)
        newBoard, _ = newState
        newScore = TDLevaluationFunction(game, newBoard, player, weights)
        #print(newScore)
        if newScore > bestScore:
            bestScore = newScore
            options = [action]
...
def playGame(game, PAI1, PAI1_starts, opponentWeights, maxEpsilon): 
    if PAI1_starts:
        ...
        myTurn = game.succ(yourTurn, TDLfeaturesMove(game, yourTurn, opponentWeights))

    while not (game.isEnd(myTurn)):
        yourTurn = game.succ(myTurn, TDLfeaturesMove(game, myTurn, opponentWeights))
...
#PAI = smartPAI(weightFile='smartPAI_v_smartPAI1000_noFlipPotential.txt')
#opponentWeightsFile = 'smartPAI_v_smartPAI1000_noFlipPotential.txt'
...
for i in range(numGames):
    ...
    if (i%10) == 0:
        print(i)
        print "White: %d" % (wins['White'])
        print "Black: %d" % (wins['Black'])
        winRate = float(wins['White']) / (wins['White'] + wins['Black'] + wins['Draw'])
        print "White's current win rate: %f" % (winRate)
