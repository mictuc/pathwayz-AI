...
def playGame(game, PAI1, PAI2):
    ...
    while not (game.isEnd(state)):
        ...
        newBoard, newPlayer = newState
        PAI2.updateWeights(game, newPlayer, board, newBoard)
        state = newState
        board, player = state
...
for i in range(numGames):
    ...
    if i%10 == 0:
        print(i)
        print 'White has won %d games' % (wins['PAI1'])
        print 'Black has won %d games' % (wins['PAI2'])
        """
        winRate = wins['white'] / 25.0
        print "white win rate: %f" % (winRate)
        wins['white'] = 0
        wins['Opponent'] = 0
        wins['Draw'] = 0
        """
    ...
    if (i%10) == 0:
        fileName = 'SmartPAI_weights_self_play%d.txt' % (i)
        f = open(fileName, 'w')
        json.dump(PAI.weights, f)
        f.close()
        
print 'White won %s percent' % (1.0 * wins['PAI1'] / numGames)
print 'Black won %s percent' % (1.0 * wins['PAI2'] / numGames)
print 'Drew %s percent' % (1.0 * wins['Draw'] / numGames)
