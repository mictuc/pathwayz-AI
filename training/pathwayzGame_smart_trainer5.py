...
class smartPAI:
    ...
    def epsilonMove(self, game, state):
        board, player = state
        actions = shuffle(game.actions(state))
        scores = []
        for action in actions:
        ...
        for i in range(len(sortedScores)):
            if (2**(i+1) - 1) / 2**(i+1) <= epsilon:
                chosenScore, chosenAction = sorted(scores, key=lambda score: score[0], reverse=True)[i]
                return chosenAction
        bestScore, bestAction = sorted(scores, key=lambda score: score[0], reverse=True)[0]
        return bestAction

def playGame(game, PAI1, PAI1_starts, opponentMove):    
    if PAI1_starts:
        myTurn = game.startState()
    else:
        yourTurn = game.startState()
        myTurn = game.succ(yourTurn, opponentMove(game, yourTurn))

    while not (game.isEnd(myTurn)):
        yourTurn = game.succ(myTurn, PAI1.epsilonMove(game, myTurn))
        if game.isEnd(yourTurn):
            PAI1.updateWeights(game, myTurn[1], myTurn[0], yourTurn[0])
            endState = yourTurn
            break
        myNextTurn = game.succ(yourTurn, opponentMove(game, yourTurn))
...
PAI = smartPAI(weightFile='SmartPAI_weights_advBaseline_opt.txt')
# gameStats = []
...
for i in range(numGames):
    ...
    if i % 2 == 0:
        winner = playGame(game, PAI, True, advancedBaselineMove)
        if winner == 'w':
            wins['PAI'] += 1
        elif winner == 'b':
            wins['Opponent'] += 1
        ...
        winner = playGame(game, PAI, False, advancedBaselineMove)
        if winner == 'w':
            wins['Opponent'] += 1
        elif winner == 'b':
            wins['PAI'] += 1
    ...
    if (i%10) == 0:
        print "PAI wins %d games" % wins['PAI'] 
        print "Opponent wins %d games" % wins['Opponent']
        print(i)
        wins['PAI'] = 0
        wins['Opponent'] = 0
        wins['Draw'] = 0
    ...
    if (i%100) == 0:
        fileName = 'SmartPAI_weights_eps_advBaseline%d.txt' % (i)
        f = open(fileName, 'w')
