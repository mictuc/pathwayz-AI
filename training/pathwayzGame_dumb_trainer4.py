...
def playGame(game, PAI):
    state = game.startState()
    oldState = None
    while not (game.isEnd(state)):
        newState = game.succ(state, baselineMove(game, state))
        if oldState != None:
            PAI.updateWeights(game, newState[1], oldState[0], newState[0])
        oldState = state
        state = newState
        ...
        state = newState
        if game.isEnd(state):
            PAI.updateWeights(game, oldState[1], oldState[0], state[0])
            break

    # state = game.startState()
    # newState2 = None
    # while not (game.isEnd(state)):
    #     newState1 = game.succ(state, PAI1.move(game, state))
    #     if newState2 != None:
    #         PAI2.updateWeights(game, newState1[1], oldNewState1[0], newState1[0])
    #     oldNewState1 = newState1
    #     if game.isEnd(newState1):
    #         PAI1.updateWeights(game, state[1], state[0], newState2[0])
    #         state = newState1
    #         break
    #     newState2 = game.succ(newState1, PAI2.move(game, newState1))
    #     PAI1.updateWeights(game, newState2[1], state[0], newState2[0])
    #     state = newState2
...
for i in range(numGames):
    ...
    if (i%10000) == 0:
        fileName = 'PAI_VS_BASELINE_weights%d.txt' % (i)
        f = open(fileName, 'w')
...
print 'BASELINE 1 won %s percent' % (1.0 * wins['BASELINE'] / numGames)
print 'dumbPAI 2 won %s percent' % (1.0 * wins['PAI'] / numGames)
