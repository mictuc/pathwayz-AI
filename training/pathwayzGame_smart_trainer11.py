...
class PathwayzGame:
    ...
    def findPathLength(self, board, player, row, col, emptyNeighbors):
        ...
        for i, j in self.surroundingPlaces(row, col):
            if board[i][j].lower() == player:
                ...
                if j == 11:
                    ...
                    self.alreadyChecked[i][j] = True
                    maxCol = self.findPathLength(board, player, i, j, emptyNeighbors)
                    if maxCol > farthestCol:
                        farthestCol = maxCol
            elif board[row][col] == player and board[i][j] == '-':
                emptyNeighbors[row][col] = 1
        return farthestCol
    ...
    def longestPath(self, board, player):
        ...
        self.alreadyChecked = [[False for i in range(12)] for j in range(8)]
        longestPath = -1
        emptyNeighbors = [[0 for i in range(12)] for j in range(8)]
        for i,j in [(i, j) for j in range(12) for i in range(8)]:
            if (board[i][j].lower() == player):
                if not self.alreadyChecked[i][j]:
                    self.alreadyChecked[i][j] = True
                    newEmptyNeighbors = [[0 for i in range(12)] for j in range(8)]
                    newPath = self.findPathLength(board, player, i, j, newEmptyNeighbors) - j
                    if newPath > longestPath:
                        longestPath = newPath
                        emptyNeighbors = newEmptyNeighbors
            # Complete path
            if longestPath == 11:
                return 12 
        return longestPath + 1
        # INSTEAD RETURN TUPLE OF (LONGESTPATH + 1, EMPTYNEIGHBORS)
        # could maybe just have an int for empty neighbors, but would need to make sure not double counting
    ...
    def smartFeatures(self, board, player):
        ...
        #print(features['my1Flip'] * 96)
        #print(features['your1Flip'] * 96)
        #print (features['myPathFlips'] * 96)
        #print (features['yourPathFlips'] * 96)
        features['myPathFlips'] = features['myPathFlips'] / (yourLongestPath + 1.0)
        features['yourPathFlips'] = features['yourPathFlips'] / (myLongestPath + 1.0)
        

        #features['myPathPerm'] = game.flipPathCount(board, game.otherPlayer(player)), / (myLongestPath + 1)
        #features['yourPathPerm'] = game.flipPathCount(board, player), / (yourLongestPath + 1)
...
PAI = smartPAI(weightFile='smartPAI_v_smartPAI3100_opt.txt')
#opponentWeightsFile = 'SmartPAI_weights_advBaseline_opt.txt'
opponentWeightsFile = 'smartPAI_v_smartPAI3100_opt.txt'
...
for i in range(numGames):
    ...
    if (i%50) == 0 and i != 0:
        ...
        print "-----------------"
        fileName = 'smartPAI_v_smartPAI_flips%d.txt' % (i)
        f = open(fileName, 'w')
