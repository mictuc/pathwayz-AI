...
# gameStats = []
PAI = dumbPAI(weightFile='SmartPAI1_weights2000.txt')
start_time = time.time()
policies = [advancedBaselineMove, minimax]
wins = {'dumbPAI':0, 'Random':0, 'Draw':0}
test = {'dumbPAI':0, 'Random':0, 'Draw':0}
numTrainingGames = 10
numTestGames = 0

for i in range(numTrainingGames+numTestGames):
    winner = playGame(game, PAI, True)
    if i < numTrainingGames:
        if winner == 'w':
            wins['Random'] += 1
        elif winner == 'b':
            wins['dumbPAI'] += 1
    ...
    if (i%1000) == 0 and i != 0:
        fileName = 'weights%d.txt' % (i)
        f = open(fileName, 'w')
        json.dump(PAI.weights, f)
        f.close()


print 'dumbPAI 1 won %s percent in training' % (1.0 * wins['dumbPAI'] / numTrainingGames)
print 'Random 2 won %s percent in training' % (1.0 * wins['Random'] / numTrainingGames)
print 'Draw %s percent in training' % (1.0 * wins['Draw'] / numTrainingGames)
print 'dumbPAI 1 won %s percent in testing' % (1.0 * test['dumbPAI'] / numTestGames)
print 'Random 2 won %s percent in testing' % (1.0 * test['Random'] / numTestGames)
print 'Draw %s percent in testing' % (1.0 * test['Draw'] / numTestGames)
print '%d games were played in %s seconds' % (numTrainingGames+numTestGames, int(time.time() - start_time))
